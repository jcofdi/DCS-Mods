...
function onRequest(sender_id, method_name, param_str, request_id)
    ...
    if not func then
        net.send_rpc_error(sender_id, request_id, 404, method_name .. " Method not found")
        return
    end
...
function onError(sender_id, request_id, error_code, error_msg)
    ...
    print(string.format("LuaRPC Error: method = %s, request = %d, error_code = %d, error_msg = %s", method or 'unknown', request_id or -1, error_code, error_msg or 'unknown'))
...
function sendRequest(handler, dest_id, method, ...)
	...
	print(string.format("rpc request %d: %s", request_id, method));
