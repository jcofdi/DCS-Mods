#ifndef BEND_SSS_GPU_HLSL
#define BEND_SSS_GPU_HLSL
...
//		(void)pixel_xy;	//unused by this implementation, avoid potential compiler warning.
	...
	groupshared float MaskData[READ_COUNT * WAVE_SIZE];
	...
	void WriteScreenSpaceShadow(DispatchParameters inParameters, int3 inGroupID, int inGroupThreadID)
	{
		...
		[unroll] for (i = 0; i < READ_COUNT; i++)
		{
			...
			depths.x = SampleMap(DepthTexture, read_xy);
			depths.y = SampleMap(DepthTexture, read_xy + offset_xy);
		#else
			...
			mask[i] = any(step(abs(read_xy * InvDepthTextureSize * 2 - 1), 1)) ? (SampleMap(StencilTexture, read_xy).g & 1) == 0 : 0;	// no grass shadows
//			mask[i] = any(step(abs(read_xy * InvDepthTextureSize * 2 - 1), 1)) ? (SampleMap(StencilTexture, read_xy).g >> STENCIL_COMPOSITION_MASK_SHIFT) == 0 : 0;
	#if USE_DEBUG
			mask[i] *= (gDev0.y > 0.5);
	#endif
